...
from utils.core import load_image, image_to_uri
...
class RetrievalQualityJudge:
    ...
    def score_retrieval_quality(
        ...
        # 构建查询摘要用于监控
        query_summary = f"{query_category}/{query_style}/{query_season}"
        ...
        if not retrieved_results:
            ...
            if METRICS_AVAILABLE:
                record_prompt_execution(
                    ...
                    prompt_version="2.0",
                    model=self.model,
                    input_summary=f"检索评估: {query_summary}",
                    output_summary="无结果",
        ...
        # 【v2.0】使用配置化的检索质量评估提示词
        if PROMPTS_AVAILABLE:
            score_prompt = PromptBuilder.build_retrieval_judge(
                ...
                if scores:
                    ...
                    if METRICS_AVAILABLE:
                        record_prompt_execution(
                            ...
                            prompt_version="2.0",
                            model=current_model,
                            input_summary=f"检索评估: {query_summary}, 结果数: {len(retrieved_results)}",
                            output_summary=f"评分: {avg_score:.1f}/10",
                            execution_time=time.time() - start_time,
                            success=True,
                            metadata={
                                "avg_score": avg_score,
